...
pub_mut!(
    gateway_status,
    GatewayStatus,
    GatewayStatus::GatewayStatusNormal
);
...
pub_mut!(
    pkt_terminate,
    [u8; 8],
    [0x04, 0x00, 0x00, 0x00, 0x03, 0x02, 0x02, 0x13]
);
...
pub fn mesh_pair_proc_effect() {
    if *get_effect_new_mesh() != 0
        || (*get_effect_new_mesh_delay_time() != 0
            && (clock_time_exceed(
                *get_effect_new_mesh_delay_time(),
                *get_mesh_pair_cmd_interval() * 1000,
            )))
    {
...
pub fn get_mesh_pair_checksum_fn(idx: u8) -> u8 {
    ...
    return (get_new_mesh_name()[i] ^ get_new_mesh_name()[i + 8])
        ^ (get_new_mesh_pwd()[i] ^ get_new_mesh_pwd()[i + 8])
        ^ (get_new_mesh_ltk()[i] ^ get_new_mesh_ltk()[i + 8]);
}
...
fn get_online_node_cnt() -> u8 {
    ...
    if *get_gateway_status() == GatewayStatus::GatewayStatusCfgUnproDev
        || *get_gateway_status() == GatewayStatus::GatewayStatusAddConfiguredDevice
    {
...
fn mesh_pair_proc() {
    ...
    if *get_default_mesh_effect_delay_ref() != 0
        && clock_time_exceed(
            *get_default_mesh_effect_delay_ref(),
    ...
    } else if *get_default_mesh_time_ref() != 0
        && clock_time_exceed(
            *get_default_mesh_time_ref(),
            *get_default_mesh_time() * 1000,
        )
    {
        /* Switch to normal mesh */
    ...
    if *get_mesh_pair_start_time() != 0
        && clock_time_exceed(
            *get_mesh_pair_start_time(),
            *get_mesh_pair_timeout() * 1000 * 1000,
        )
    {
        //mesh pair time out
        _pair_load_key();
    ...
    if *get_pair_setting_flag() == PairState::PairSetMeshTxStart
        && *get_mesh_pair_state() == MeshPairState::MeshPairName1
        && get_online_node_cnt() == 1
    {
        ...
        && clock_time_exceed(*get_mesh_pair_time(), *get_mesh_pair_cmd_interval() * 1000)
    {
        ...
        if *get_pair_setting_flag() == PairState::PairSetMeshTxStart {
            ...
            get_mesh_pair_checksum()
                .iter_mut()
                .enumerate()
                .for_each(|(i, x)| *x = get_mesh_pair_checksum_fn(i as u8));
        } else if *get_pair_setting_flag() == PairState::PairSetMeshRxDone {
            let mut effect_flag = *get_mesh_pair_notify_rsp_mask() == [0; 32];
            if !effect_flag
                && clock_time_exceed(*get_mesh_pair_start_time(), MESH_PAIR_NOTIFY_TIMEOUT * 1000)
            {
