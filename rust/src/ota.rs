...
use sdk::light::{_get_fw_version, _is_add_packet_buf_ready, _is_master_sending_ota_st, _is_mesh_ota_slave_running, _light_sw_reboot, _mesh_ota_master_start, _mesh_ota_master_start_firmware_from_backup, _rf_link_add_tx_packet, _rf_link_slave_read_status_stop, _rf_ota_save_data, APP_OTA_HCI_TYPE, get_app_ota_hci_type, get_cur_ota_flash_addr, get_pair_login_ok, get_rf_slave_ota_busy, get_rf_slave_ota_finished_flag, get_rf_slave_ota_terminate_flag, get_rf_slave_ota_timeout_def_s, get_slave_read_status_busy, get_tick_per_us, mesh_ota_dev_info_t, MESH_OTA_LED, mesh_ota_pkt_start_command_t, OtaState, RecoverStatus, rf_packet_att_data_t, set_cur_ota_flash_addr, set_mesh_ota_master_100_flag, set_rf_slave_ota_busy, set_rf_slave_ota_finished_flag, set_rf_slave_ota_terminate_flag, set_rf_slave_ota_timeout_s, START_UP_FLAG};
...
impl OtaManager {
    ...
    pub fn handle_ota_update__attribute_ram_code() {
        ...
        unsafe {
            if *(OtaManager::FLASH_ADR_OTA_READY_FLAG as *const u8) != OtaManager::FLASH_OTA_READY_FLAG {
                // Nothing to do
                return;
            }
        }
    ...
    pub fn is_ota_area_valid(&self) -> bool {
        ...
        for i in 0..OtaManager::ERASE_SECTORS_FOR_OTA {
            flash_read_page(*get_flash_adr_light_new_fw() + i * 0x1000, 4, buf.as_mut_ptr());
    ...
    pub fn erase_ota_data(&self) {
        for i in 0..OtaManager::ERASE_SECTORS_FOR_OTA {
            flash_erase_sector(*get_flash_adr_light_new_fw() + (OtaManager::ERASE_SECTORS_FOR_OTA - 1 - i) * 0x1000);
        }
    }
    ...
    pub fn mesh_ota_master_100_flag_check(&self)
    {
        ...
        if val & RecoverStatus::MeshOtaMaster100 as u8 != 0 {
            ...
            analog_write__attribute_ram_code(REGA_LIGHT_OFF, val & !(RecoverStatus::MeshOtaMaster100 as u8));
        }
    }
    ...
    pub fn rf_link_slave_data_ota(&mut self, data: &[u32])
    {
        ...
        get_buff_response()[(self.slave_ota_data_cache_idx%16) as usize][0..data.len()].copy_from_slice(&data[0..]);
                    ...
                    if cur_idx == 0 {
                        ...
                        if *get_cur_ota_flash_addr() != 0 {
                            ...
                            if self.need_check_type == 1 {
                                self.fw_check_val = (p.dat[(n_data_len +2) as usize] as u16 | (p.dat[(n_data_len +3) as usize] as u16) <<8) as u32;
                            }
                        ...
                        // ota fw check
                        if cur_idx == 1 {
                            self.ota_pkt_total = ((((p.dat[10] as u32) |( ((p.dat[11] as u32) << 8) & 0xFF00) | (((p.dat[12] as u32) << 16) & 0xFF0000) | (((p.dat[13] as u32) << 24) & 0xFF000000)) + 15)/16) as u16;
                            ...
                            if self.fw_check_val != ((p.dat[2] as u32) |( ((p.dat[3] as u32) << 8) & 0xFF00) | (((p.dat[4] as u32) << 16) & 0xFF0000) | (((p.dat[5] as u32) << 24) & 0xFF000000)) {
                                set_cur_ota_flash_addr(0);
                                self.ota_pkt_cnt = 0;
                                self.ota_rcv_last_idx = 0;
                                reset_flag = OtaState::ERROR;
                        ...
                        {
                            if *get_cur_ota_flash_addr() + 16 > (OtaManager::FW_SIZE_MAX_K * 1024) { // !is_valid_fw_len()
                                reset_flag = OtaState::ERROR;
                            } else {
    ...
    pub fn rf_ota_set_flag(&self) {
        ...
        flash_write_page (OtaManager::FLASH_ADR_OTA_READY_FLAG, 4, addr_of!(flag) as *mut u8);
    }
    ...
    fn rf_link_slave_ota_finish_handle(&mut self)		// poll when ota busy in bridge
    {
        ...
        if *get_rf_slave_ota_finished_flag() != OtaState::CONTINUE {
            ...
            if !*get_rf_slave_ota_terminate_flag() && (clock_time() - self.rf_slave_ota_finished_time) > 2000*1000 * *get_tick_per_us() {
    ...
    fn mesh_ota_master_start_firmware(&self, p_dev_info: *const mesh_ota_dev_info_t, new_fw_adr: u32)
    {
...
mod wrappers {
    ...
    use std::slice;
    use app;
    use sdk::light::{mesh_ota_dev_info_t, MESH_OTA_LED, OtaState, rf_packet_att_data_t};
    ...
    {
        app().ota_manager.rf_link_slave_ota_finish_led_and_reboot(st);
    }
    ...
    {
        app().ota_manager.mesh_ota_master_start_firmware(p_dev_info, new_fw_adr);
    }
