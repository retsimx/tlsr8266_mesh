    ...
    mesh_node_st.iter().enumerate().for_each(|(idx, val)| {
        ...
        if mask & (1 << mask_bit) != 0 {
            ...
            params[params_idx..params_idx + MESH_NODE_ST_VAL_LEN].copy_from_slice(
                bytemuck::bytes_of(&mesh_node_st[idx].val)
            );
...
mod tests {
    ...
    use crate::sdk::ble_app::rf_drv_8266::{
        mock_rf_stop_trx, mock_rf_set_ble_access_code, mock_rf_set_ble_crc_adv,
        mock_rf_set_ble_channel, mock_rf_set_rxmode
    };
    
    // Import mock functions for register operations
    use crate::sdk::mcu::register::{
        mock_read_reg_system_tick, mock_write_reg_system_tick_irq, 
    ...
    fn setup_mock_mesh_node(idx: usize, dev_adr: u8, sn: u8, tick: u16, par: [u8; MESH_NODE_ST_PAR_LEN]) {
        let mut mesh_node_mask = MESH_NODE_MASK.lock();
        let mut mesh_node_st = MESH_NODE_ST.lock();
    ...
    fn test_mesh_node_report_status_disabled() {
        ...
        assert_eq!(params, [0xFF; 20], "Output buffer should remain unchanged when reporting is disabled");
    }
    ...
    fn test_mesh_node_report_status_no_pending() {
        ...
        // Verify no statuses were reported
        assert_eq!(result, 0, "Should return 0 when no nodes have pending status");
        ...
        assert_eq!(&params[0..expected_clear_bytes], &vec![0u8; expected_clear_bytes][..], 
    ...
    fn test_mesh_node_report_status_single_online_node() {
        ...
        assert_eq!(mesh_node_mask[0] & 1, 0, "Mask bit should be cleared after reporting");
    }
    ...
    fn test_mesh_node_report_status_single_offline_node() {
        ...
        assert_eq!(params[1], 0x00, "Sequence number should be 0 for offline node");
        ...
        let mesh_node_mask = MESH_NODE_MASK.lock();
        assert_eq!(mesh_node_mask[0] & (1 << 2), 0, "Mask bit should be cleared after reporting");
    }
    ...
    fn test_mesh_node_report_status_multiple_nodes() {
        ...
        assert_eq!(params[9], 0x00, "Third node sequence number should be 0 for offline");
        ...
        let mesh_node_mask = MESH_NODE_MASK.lock();
        assert_eq!(mesh_node_mask[0] & 0b1011, 0, "All reported node mask bits should be cleared");
    }
    ...
    fn test_mesh_node_report_status_buffer_limit() {
        ...
        let mesh_node_mask = MESH_NODE_MASK.lock();
        assert_eq!(mesh_node_mask[0] & (1 << 2), 1 << 2, "Third node should still be pending");
        assert_eq!(mesh_node_mask[0] & 0b11, 0, "First two nodes should be cleared");
    }
    ...
    fn test_configure_rf_for_mesh_listening() {
        ...
        assert_eq!(BLE_PERIPHERAL_LINK_STATE.get(), BlePeripheralLinkState::Mesh,
    ...
    fn test_configure_rf_for_mesh_listening_channel_cycling() {
        ...
        for i in 0..SYS_CHN_LISTEN.len() * 2 { // Test two full cycles
    ...
    fn test_handle_mesh_listening_state_periodic_advertising() {
        ...
        assert_eq!(MESH_LISTEN_CYCLE_COUNT.get(), ADV_INTERVAL2LISTEN_INTERVAL as u32,
            "Listen cycle count should be incremented");
        
        // Verify advertising was enabled (cycle count % ADV_INTERVAL2LISTEN_INTERVAL != 0)
        assert_eq!(BLE_ADVERTISING_ENABLED.get(), false, // Should be false when count % interval == 0
    ...
    fn test_handle_mesh_listening_state_continue_listening() {
        ...
        // Verify listen cycle count was incremented
        assert_eq!(MESH_LISTEN_CYCLE_COUNT.get(), 2,
            "Listen cycle count should be incremented");
        ...
        assert_eq!(BLE_ADVERTISING_ENABLED.get(), should_advertise,
            "Advertising state should match interval calculation");
        assert_eq!(MESH_DEVICE_ONLINE_STATUS.get(), should_online_status,
    ...
    fn test_handle_mesh_listening_state_continue_listening_no_action_needed() {
        ...
        // Verify listen cycle count was incremented
        assert_eq!(MESH_LISTEN_CYCLE_COUNT.get(), 8,
        ...
        assert_eq!(BLE_ADVERTISING_ENABLED.get(), false,
            "Advertising should not be enabled when cycle count % 4 == 0");
        assert_eq!(MESH_DEVICE_ONLINE_STATUS.get(), false,
    ...
    fn test_handle_mesh_listening_state_online_status_already_scheduled() {
        ...
        assert_eq!(MESH_LISTEN_CYCLE_COUNT.get(), 5,
        ...
        assert_eq!(MESH_DEVICE_ONLINE_STATUS.get(), true,
