use crate::sdk::mcu::register::{
    read_reg_aes_ctrl, read_reg_aes_data, write_reg_aes_ctrl, write_reg_aes_data, write_reg_aes_key,
};
...
#[cfg_attr(test, mry::mry)]
pub fn aes_att_encryption_packet(
    key: &[u8; 16],
    ivm: &[u8; 8],
    mic_output: &mut [u8],
    packet_data: &mut [u8],
...
mod tests {
    ...
    use crate::sdk::mcu::crypto::{
        mock_aes_att_decryption, mock_aes_att_encryption, mock_aes_ll_encryption,
    };
    use crate::sdk::mcu::register::{
        mock_read_reg_aes_ctrl, mock_read_reg_aes_data, mock_write_reg_aes_ctrl,
        mock_write_reg_aes_data, mock_write_reg_aes_key,
    };
    use crate::state::PAIR_CONFIG_PWD_ENCODE_SK;
    use mry::{self, Any};
    ...
    fn test_aes_ll_encryption_basic() {
        ...
        // Using returns_with with a static counter to return different values on each call
        {
            ...
            mock_read_reg_aes_ctrl().returns_with(move || unsafe {
        ...
        // Using returns_with with a static counter to return different values on each call
        {
            ...
            mock_read_reg_aes_data().returns_with(move || unsafe {
        ...
        assert_eq!(
            result,
            [
                0x23, 0x22, 0x21, 0x20, 0x27, 0x26, 0x25, 0x24, 0x2B, 0x2A, 0x29, 0x28, 0x2F, 0x2E,
                0x2D, 0x2C
            ]
        );
    }
    ...
    fn test_aes_ll_encryption_decrypt_mode() {
        ...
        assert_eq!(
            result,
            [
                0x13, 0x12, 0x11, 0x10, 0x17, 0x16, 0x15, 0x14, // Check the returned result
    ...
    fn test_aes_att_decryption_packet_small() {
        ...
        let first_encryption = [
            0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x4E,
            0x4F, 0x50,
        ];
        let second_encryption = [
        ...
        mock_aes_att_encryption(Any, Any).returns_with(move |_, _| unsafe {
    ...
    fn test_aes_att_decryption_packet_validation_success() {
        ...
        mock_aes_att_encryption(Any, Any).returns_with(move |_, _| {
            unsafe {
                CALL_COUNT += 1;
                if CALL_COUNT == 3 {
                    // On the third call (final validation check)
                    final_encryption
                } else {
                    [
    ...
    fn test_aes_att_decryption_packet_validation_failure() {
        ...
        mock_aes_att_encryption(Any, Any).returns_with(move |_, _| {
            unsafe {
                CALL_COUNT += 1;
                if CALL_COUNT == 2 {
                    // On the second call (final validation check)
                    final_encryption
                } else {
                    [
                        0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C,
    ...
    fn test_aes_att_decryption_packet_alignment_boundaries() {
        ...
        for i in 0..16 {
            assert_eq!(
                packet_data[i],
                original_packet_data[i] ^ encrypt_results[0][i]
            );
        }
    }
    ...
    fn test_aes_att_decryption_packet_complex_encryption_flow() {
        ...
        let encryption_results = [
            ...
            [
                0x31, 0x32, 0x33, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6A, 0x6B, 0x6C, 0x6D, 0x6E,
                0x6F, 0x70,
            ],
        ];
        static mut CALL_COUNT: usize = 0;
    ...
    fn test_aes_att_encryption_packet_small() {
        ...
        for i in 0..packet_data.len() {
            assert_eq!(
                packet_data[i],
                original_packet_data[i] ^ final_result[i & 0xf]
            );
        }
    ...
    fn test_aes_att_encryption_packet_large() {
        ...
        let first_result_block = [
            0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7A, 0x7B, 0x7C, 0x7D, 0x7E,
            0x7F, 0x80,
        ];
        let second_result_block = [
        ...
        for i in 0..16 {
            assert_eq!(
                packet_data[i],
                original_packet_data[i] ^ first_result_block[i]
            );
        }
        ...
        for i in 16..packet_data.len() {
            assert_eq!(
                packet_data[i],
                original_packet_data[i] ^ second_result_block[i & 0xf]
            );
        }
