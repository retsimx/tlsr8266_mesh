...
{
    unsafe {
        aes_att_encryption(&state.pair_config_pwd_encode_sk, &*addr_of!(*password), password);
    }
}
...
{
    unsafe {
        aes_att_decryption(&state.pair_config_pwd_encode_sk, &*addr_of!(*password), password);
    }
}
