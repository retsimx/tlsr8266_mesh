...
//////////////// adc sample data convert to voltage(mv) ////////////////
	...
	if(batt_vol_mv < alram_vol_mv){
		...
		#else
		LOG_BATTERY_CHECK_DEBUG(0,0,"battery low, loop:%d, current:%d, thres:%d\r\n", loop_flag, batt_vol_mv, alram_vol_mv);
		battery_power_low_handle(loop_flag);
		ret_slept_flag = 1;
		#endif
	}else{
	    // rega_light_off can not be cleared here, because it will be used in light pwm init.
	}
...
_attribute_no_inline_ void app_battery_power_check_and_sleep_handle(int loop_flag)
{
    ...
    if(loop_flag){
    	...
    	if(battery_get_detect_enable() && (clock_time_exceed(lowBattDet_tick, VBAT_ALRAM_CHECK_INTERVAL_MS * 1000)
    	    ...
    	    #else
    	    if((0 == lowBattDet_tick) && (analog_read(rega_light_off) & LOW_BATT_FLG)){
                analog_write(rega_light_off,  analog_read(rega_light_off)  & (~ (LOW_BATT_FLG|FLD_LIGHT_OFF)));  //clear
    	    }
    	    #endif
    		lowBattDet_tick = clock_time();
            app_battery_power_check(alarm_thres, loop_flag);  //2.0 V
            if(batt_vol_mv < 2500){
                LOG_BATTERY_CHECK_DEBUG(0,0,"battery loop normal, current:%d, ana reg0:0x%x\r\n", batt_vol_mv, analog_read(rega_light_off));
            }
        }
    }else{ // user init
        if(analog_read(rega_light_off) & LOW_BATT_FLG){
            app_battery_power_check(alarm_thres + 200, loop_flag);  //2.2 V
        }else{
            app_battery_power_check(alarm_thres, loop_flag);  //2.0 V
        }
		LOG_BATTERY_CHECK_DEBUG(0,0,"battery user init normal, current:%d, ana reg0:0x%x\r\n", batt_vol_mv, analog_read(rega_light_off));
    }
