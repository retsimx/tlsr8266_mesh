...
{
	#if SWITCH_FW_ENABLE
	u32 flash_adr_new_fw = (*(u8 *)FLASH_ADR_BOOT_FLAG == 0xa5)?FLASH_ADR_LIGHT_SIG_MESH:FLASH_ADR_LIGHT_NEW_FW;
	#else
	u32 flash_adr_new_fw = FLASH_ADR_LIGHT_NEW_FW;
	#endif
	...
	int n;
	#if SWITCH_FW_ENABLE
	if(flash_adr_new_fw != FLASH_ADR_LIGHT_NEW_FW){//backup current running fw
		flash_read_page (0, 256, buff);
		n = *(u32 *)(buff + 0x18);
		if(n <= FLASH_ADR_LIGHT_NEW_FW){
			
	        for (int i=0; i<n; i+=256)
	        {
	            flash_read_page (i, 256, buff);
	            flash_write_page (FLASH_ADR_LIGHT_NEW_FW+i, 256, buff);
	        }
		}
	}
    #endif
